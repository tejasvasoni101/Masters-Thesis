...
for peri = 1:div
for widi = 1:div      %for loop for width metal
for heig = 1:div      %fo height
    ...
    width = [1 0.05 + widi/(div)*Period(1)];  %from 1.05 to 1.05+1
...
for ind = 1:length(lambda)
    ...
    [Ref(ind), Tran(ind)] = RCWA_Multi_TM(N, e_m, e_d, f1, f2, Period, d, e, lambda(ind), theta, Num_ord); 
      ...
      if sz(2)>1             % atleast 2 peak found in a given plot for a design
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
peak1pos=[];
peak2pos=[];
finalfiltered=[];
...
d = [0.02 (0.02 + ((heig)/div)*0.08)];                                           % thickness of each layer from front to back, [um]
N = length(d);                                   % # of layers
Period(1:N) = 0.4+(peri/div)*1.2;                          % Period of gratings for each layer, [um]
width = [1 0.05 + widi/(div)*Period(1)];                                    % width of metal strips, [um]
psi = width/Period(1);                     % filling ratio of grating layer
f1 = [0 0];                                              % normalized position for left-end of metal strip
f2 = [psi];                                           % normalized position for right-end of metal strip
lastval=1000;
...
[pks,locs] = findpeaks(Tran,'MinPeakHeight',0.2,'MinPeakProminence',0.05,'MaxPeakWidth',20,'MinPeakDistance',10)
si = size(locs);
if(si(2)<2)
...
if(maxcontinuousshift>=10) % 3 DENOTES 3 WORKING RANGE LIKE 1.33 TO 1.35
       ...
       numb(end+1) =(div*div*(peri-1))+(div*(widi-1))+heig ;   %numb coresp to the row in temp which is selected
       phi=[hi;wi];
       phi=[phi;pie];
       phi=[phi;numb];       %phi contains all numb, w,h,p
