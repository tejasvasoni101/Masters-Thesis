...
function [Ref,Tran]=RCWA_Multi_TM(N, e_m, e_d, f1, f2, Period, d, e, lambda, theta, Num_ord)
...
%        Num_ord: number for the highest diffraction order
    ...
    for ind=1:N
        [Q{ind},V{ind},W{ind}] = Matrix_Gen_TM(e_m(ind), e_d(ind), f1(ind), f2(ind), Period(ind), e(1), lambda, theta, Num_ord);
